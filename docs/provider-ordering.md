...
For supported formats and the collector's failure and fallback behavior, see [How it works](../README.md#how-it-works).

Display-name fallbacks do not establish sign-in; the same structured state rules above apply without display or provenance metadata. New or unknown provenance never establishes auth by itself.
